...
function renderShipList() {
  ...
  for (const ship of SHIPS) {
    ...
    const shipOrientation = selected ? state.orientation : "Horizontal";
    card.innerHTML = `
      ...
      <div class="ship-card-actions">
        ${!placed && state.phase === "placement" ? `<button class="ship-action-btn" data-select="${ship.name}">Seleccionar</button>
        <button class="ship-action-btn" data-rotate-ship="${ship.name}">Rotar barco</button>` : ""}
      </div>
      <div class="ship-mini-preview ${shipOrientation === "Horizontal" ? "horizontal" : "vertical"}">
  ...
  container.querySelectorAll("[data-select]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      selectShip(btn.dataset.select);
    });
  });

  container.querySelectorAll("[data-rotate-ship]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const shipName = btn.dataset.rotateShip;
      state.selectedShip = shipName;
      state.dragShip = null;
      ...
      setStatus(`${shipName} seleccionado. Orientación actual: ${state.orientation}.`);
      renderAll();
    });
  });
}
...
function createBoardGrid(container, boardType) {
  ...
  for (let r = 0; r < BOARD_SIZE; r++) {
    ...
    for (let c = 0; c < BOARD_SIZE; c++) {
      ...
      const canHoverPlayer = boardType === "player" && state.phase === "placement";
      const canHoverEnemy = boardType === "enemy" && state.phase === "battle" && state.turn === "Jugador" && state.playerShots[r][c] === 0;
      if (canHoverPlayer || canHoverEnemy) cell.classList.add("hoverable");
      if (canHoverEnemy) cell.classList.add("enemy-hover");
      const isLastPlayerShot = boardType === "enemy" && state.lastPlayerShot && state.lastPlayerShot.row === r && state.lastPlayerShot.col === c;
      const isLastAiShot = boardType === "player" && state.lastAiShot && state.lastAiShot.row === r && state.lastAiShot.col === c;
      if (isLastPlayerShot || isLastAiShot) cell.classList.add("last-shot");
      ...
      if (boardType === "player") {
        ...
        const aiMark = state.aiMemory[r][c];
        if (value === 0) {
...
function renderMeta() {
  ...
  if (startBtn) {
    startBtn.disabled = !allPlayerShipsPlaced() || state.phase !== "placement";
    startBtn.classList.toggle("hidden", state.phase !== "placement");
  }
  if (resetBtn) {
    resetBtn.classList.toggle("hidden", state.phase === "placement");
  }
  ...
  if (indicator) {
    ...
    if (state.phase === "placement") {
      ...
      indicator.textContent = "Preparación";
    } else if (state.phase === "battle") {
      if (state.turn === "Jugador") {
        indicator.classList.add("player");
      ...
      indicator.textContent = "Partida finalizada";
    }
  }
}
