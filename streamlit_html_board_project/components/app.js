...
function renderShipList() {
  ...
  for (const ship of SHIPS) {
    ...
    card.innerHTML = `
      ...
      <div class="ship-card-actions">
        <button class="mini-btn rotate-ship-btn" data-rotate="${ship.name}">Rotar barco</button>
  ...
  container.querySelectorAll("[data-rotate]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      state.selectedShip = btn.dataset.rotate;
      state.dragShip = null;
      state.orientation = state.orientation === "Horizontal" ? "Vertical" : "Horizontal";
      clearPreview();
      setStatus(`${btn.dataset.rotate} seleccionado. Orientación actual: ${state.orientation}.`);
      renderAll();
    });
  });
}
...
function updateResponsiveBoardSize() {
  ...
  root.style.setProperty('--coord-size', '28px');
