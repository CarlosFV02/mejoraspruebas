...
function updateResponsiveBoardSize() {
  ...
  const playerPanel = document.querySelector('.player-board-panel');
  const enemyPanel = document.querySelector('.enemy-board-panel');
  const placement = state.phase === 'placement';

  const visiblePanels = [playerPanel, enemyPanel].filter((panel) => {
    if (!panel) return false;
    if (placement && panel.classList.contains('enemy-board-panel')) return false;
  ...
  const panelWidth = Math.min(...visiblePanels.map((panel) => panel.clientWidth || 0).filter(Boolean));
  const panelPadding = 44;
  const available = Math.max(320, panelWidth - panelPadding);
  const gap = state.phase === 'battle' || state.phase === 'finished' ? 3 : 4;
  const coord = available < 430 ? 20 : 24;
  const rawCell = Math.floor((available - coord - (10 * gap)) / 10);
  let cell = rawCell;

  if (state.phase === 'placement') {
    cell = Math.max(24, Math.min(46, cell));
  } else {
    cell = Math.max(22, Math.min(36, cell));
  }
